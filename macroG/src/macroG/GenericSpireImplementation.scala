...
// Imports used for Generic Spire Implementation
...
// This part consists on isolating the needed spire implementation for this benchmark using type erasure 
// for generics
...
trait BitString[ A] extends BooleanAlgebra[A] {
  def signed: Boolean
  def width: Int
...
trait CRing[ A] extends Ring[A] with MultiplicativeCMonoid[A]
...
//Rig
trait Rig[ A] extends Semiring[A] with AdditiveMonoid[A] with MultiplicativeMonoid[A] {
...
trait MultiplicativeAbGroup[ A] extends MultiplicativeGroup[A] with MultiplicativeCMonoid[A] {
 ...
 */
trait IsReal[ A] extends Order[A] with Signed[A] {
  def ceil(a: A): A
  def floor(a: A): A
  def round(a: A): A
...
object IsReal {
  def apply[ A](implicit A: IsReal[A]): IsReal[A] = A
}
...
trait Order[ A] extends Eq[A] {
  ...
  override def on[ B](f: B => A): Order[B] = new MappedOrder(this)(f)

  def reverse: Order[A] = new ReversedOrder(this)
}

private class MappedOrder[ A,  B](order: Order[B])(f: A => B) extends Order[A] {
  def compare(x: A, y: A) = order.compare(f(x), f(y))
}

private class ReversedOrder[ A](order: Order[A]) extends Order[A] {
...
object Order {
  ...
  def by[ A,  B](f: A => B)(implicit o: Order[B]): Order[A] = o.on(f)

  def from[ A](f: (A, A) => Int): Order[A] = new Order[A] {
...
trait Eq[ A] {
  ...
  def on[ B](f:B => A): Eq[B] = new MappedEq(this)(f)
}

private class MappedEq[ A,  B](eq: Eq[B])(f: A => B) extends Eq[A] {
...
object Eq {
  ...
  def by[ A,  B](f:A => B)(implicit e:Eq[B]): Eq[A] = new MappedEq(e)(f)
}
...
final case class Complex[ T](real: T, imag: T)
...
object VectorSpace {
  @inline final def apply[V,  R](implicit V: VectorSpace[V, R]) = V
}
...
object Module {
  @inline final def apply[V,  R](implicit V: Module[V, R]) = V

  implicit def IdentityModule[ V](implicit ring: Ring[V]) = {
...
trait IdentityModule[ V] extends Module[V, V] {
...
final case class ZModule[V](vector: Group[V]) extends Module[V, Int] {
 ...
 final  class ArrayVectorSpace[ A: ClassTag: Field]
...
final  class ArrayModule[ A: ClassTag: Ring]
...
final class ArrayOrder[ A: Order]
...
final class ArrayMonoid[ A: ClassTag]
...
trait ArrayInstances0 {
  ...
  implicit def ArrayModule[ A: NI0: ClassTag: Ring]: Module[Array[A], A] =
    new ArrayModule[A]
}
...
trait ArrayInstances1 extends ArrayInstances0 {
  ...
  implicit def ArrayVectorSpace[ A: NI1: ClassTag: Field]: VectorSpace[Array[A], A] =
    new ArrayVectorSpace[A]

  implicit def ArrayEq[ A: Eq]: Eq[Array[A]] =
    new ArrayEq[A]
}

trait ArrayInstances2 extends ArrayInstances1 {
  //implicit def ArrayInnerProductSpace[(Float, Double) A: Field: ClassTag]: InnerProductSpace[Array[A], A] =
...
//trait ArrayInstances3 extends ArrayInstances2 {
  //implicit def ArrayNormedVectorSpace[(Float, Double) A: Field: NRoot: ClassTag]: NormedVectorSpace[Array[A], A] =
