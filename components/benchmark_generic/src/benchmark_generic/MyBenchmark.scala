...
object InsertionSort extends Sort {
  final def sort[ A:Order:ClassTag](data:Array[A]) =
    sort(data, 0, data.length)

  final def sort[ A](data:Array[A], start:Int, end:Int)
...
object QuickSort {
  ...
  final def sort[ A:Order:ClassTag](data:Array[A]) = qsort(data, 0, data.length - 1)
  final def qsort[ A]
...
object Sorting {
  final def sort[ A:Order:ClassTag](data:Array[A]) = QuickSort.sort(data)

  final def insertionSort[ A:Order:ClassTag](data:Array[A]) = InsertionSort.sort(data)
  final def mergeSort[ A:Order:ClassTag](data:Array[A]) = MergeSort.sort(data)
  final def quickSort[ K:Order:ClassTag](data:Array[K]) = QuickSort.sort(data)
}
