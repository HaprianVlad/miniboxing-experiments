...
class ArrayOrderBenchmarks extends MyBenchmark {
  ...
  def timeAddGeneric(reps: Int) = run(reps) {implicits.ArrayModule[Int](macroG.NoImplicit.noImplicit0[macroG.VectorSpace[Array[Int],Int]], (ClassTag.Int: scala.reflect.ClassTag[Int]), implicits.IntAlgebra).plus(a, b)} 
