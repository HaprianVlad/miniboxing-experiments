...
class ArrayOrderBenchmarks extends MyBenchmark {
  ...
  def timeAddGeneric(reps: Int) = run(reps) {implicits.ArrayModule[Int](macroM.NoImplicit.noImplicit0[macroM.VectorSpace[Array[Int],Int]], (ClassTag.Int: scala.reflect.ClassTag[Int]), implicits.IntAlgebra).plus(a, b)} 
