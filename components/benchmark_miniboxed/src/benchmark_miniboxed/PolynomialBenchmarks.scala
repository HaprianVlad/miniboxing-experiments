...
//////////////////////////////////////////////////////////////////////////////////////////////////
//Term
case class Term[@miniboxed C](coeff: C, exp: Int) { lhs =>
...
object Term {
  ...
  def fromTuple[@miniboxed C](tpl: (Int, C)): Term[C] =
    Term(tpl._2, tpl._1)
  def zero[@miniboxed C](implicit r: Semiring[C]): Term[C] =
    Term(r.zero, 0)
  def one[@miniboxed C](implicit r: Rig[C]): Term[C] =
    Term(r.one, 0)
...
case class PolySparse[@miniboxed C]  (val exp: Array[Int], val coeff: Array[C])
...
object PolySparse {
 final def dense2specializedarse[@miniboxed C: Semiring: Eq: ClassTag](poly: PolyDense[C]): PolySparse[C] = {
    val cs = poly.coeffs
   ...
   final def safe[@miniboxed C: Semiring: Eq: ClassTag]
 ...
 implicit object myOrderInt extends Order[Int]{
  ...
  final def apply[@miniboxed C: Semiring: Eq: ClassTag](data: Map[Int,C]): PolySparse[C] = {
  ...
  final def apply[@miniboxed C: Semiring: Eq: ClassTag](poly: Polynomial[C]): PolySparse[C] = {
    poly match {
  ...
  final def zero[@miniboxed C: Semiring: Eq: ClassTag]: PolySparse[C] =
    new PolySparse(new Array[Int](0), new Array[C](0))

   final def multiplyTerm[@miniboxed C: Semiring: Eq: ClassTag](poly: PolySparse[C], c: C, e: Int): PolySparse[C] = {
    val exp = poly.exp
   ...
   final def multiplySparse[@miniboxed C: Semiring: Eq: ClassTag]
   ...
   final def countSumTerms[@miniboxed C]
   ...
   final def quotmodSparse[@miniboxed C: Field: Eq: ClassTag]
...
class PolyDense[@miniboxed C]  (val coeffs: Array[C])
    (implicit val ct: ClassTag[C]) extends Polynomial[C]  { lhs =>
...
object Polynomial extends PolynomialInstances {
  ...
  def dense[@miniboxed C: Semiring: Eq: ClassTag](coeffs: Array[C]): PolyDense[C] = {
    var i = coeffs.length
  ...
  def specializedarse[@miniboxed C: Semiring: Eq: ClassTag](data: Map[Int, C]): PolySparse[C] =
    PolySparse(data)

  def apply[@miniboxed C: Semiring: Eq: ClassTag](data: Map[Int, C]): PolySparse[C] =
    specializedarse(data)

  def apply[@miniboxed C: Semiring: Eq: ClassTag](terms: Iterable[Term[C]]): PolySparse[C] =
    specializedarse(terms.map(_.toTuple)(collection.breakOut))

  def apply[@miniboxed C: Semiring: Eq: ClassTag](c: C, e: Int): PolySparse[C] =
  ...
  def zero[@miniboxed C: Eq: Semiring: ClassTag]: Polynomial[C] =
    PolySparse.zero[C]
  def constant[@miniboxed C: Eq: Semiring: ClassTag](c: C): Polynomial[C] =
    if (c == Semiring[C].zero) zero[C] else Polynomial(Map(0 -> c))
  def linear[@miniboxed C: Eq: Semiring: ClassTag](c: C): Polynomial[C] =
    if (c == Semiring[C].zero) zero[C] else Polynomial(Map(1 -> c))
  def quadratic[@miniboxed C: Eq: Semiring: ClassTag](c: C): Polynomial[C] =
    if (c == Semiring[C].zero) zero[C] else Polynomial(Map(2 -> c))
  def cubic[@miniboxed C: Eq: Semiring: ClassTag](c: C): Polynomial[C] =
    if (c == Semiring[C].zero) zero[C] else Polynomial(Map(3 -> c))
  def one[@miniboxed C: Eq: Rig: ClassTag]: Polynomial[C] =
    constant(Rig[C].one)
  def x[@miniboxed C: Eq: Rig: ClassTag]: Polynomial[C] =
    linear(Rig[C].one)
  def twox[@miniboxed C: Eq: Rig: ClassTag]: Polynomial[C] =
  ...
  final def specializedlit[@miniboxed C: ClassTag](poly: Polynomial[C]): (Array[Int], Array[C]) = {
...
trait Polynomial[@miniboxed C] { lhs =>
...
trait PolynomialSemiring[@miniboxed C]
...
trait PolynomialRig[@miniboxed C] extends PolynomialSemiring[C]
...
trait PolynomialRng[@miniboxed C] extends PolynomialSemiring[C]
...
trait PolynomialRing[@miniboxed C] extends PolynomialRng[C]
...
trait PolynomialEuclideanRing[@miniboxed C] extends PolynomialRing[C]
...
trait PolynomialEq[@miniboxed C] extends Eq[Polynomial[C]] {
  implicit def scalar: Semiring[C]
  implicit def eq: Eq[C]
  implicit def ct: ClassTag[C]
...
trait PolynomialInstances0 {
  implicit def semiring[@miniboxed C: ClassTag: Semiring: Eq] =
  ...
  implicit def eq[@miniboxed C: ClassTag: Semiring: Eq] =
    new PolynomialEq[C] {
      val scalar = Semiring[C]
      val eq = Eq[C]
      val ct = implicitly[ClassTag[C]]
...
trait PolynomialInstances1 extends PolynomialInstances0 {
  implicit def rig[@miniboxed C: ClassTag: Rig: Eq] =
    new PolynomialRig[C] {
      val scalar = Rig[C]
      val eq = Eq[C]
      val ct = implicitly[ClassTag[C]]
  ...
  implicit def rng[@miniboxed C: ClassTag: Rng: Eq] =
    new PolynomialRng[C] {
      val scalar = Rng[C]
      val eq = Eq[C]
      val ct = implicitly[ClassTag[C]]
...
trait PolynomialInstances2 extends PolynomialInstances1 {
  implicit def ring[@miniboxed C: ClassTag: Ring: Eq] =
...
trait PolynomialInstances3 extends PolynomialInstances2 {
  implicit def euclideanRing[@miniboxed C: ClassTag: Field: Eq] =
...
trait Integral[@miniboxed A] extends EuclideanRing[A] with ConvertableFrom[A] with ConvertableTo[A] with IsReal[A]
...
// Ring Algebra
 ...
 trait RingAlgebra[V, @miniboxed R] extends Module[V, R] with Rng[V]
...
trait FieldAlgebra[V, @miniboxed F] extends RingAlgebra[V, F] with VectorSpace[V, F]
...
def qsortBy[@miniboxed B](f: A => B)(implicit ev: Order[B], ct: ClassTag[A]): Unit = {
